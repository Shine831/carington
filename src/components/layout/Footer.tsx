  ...
  return (
    <footer className="bg-[var(--charcoal)] text-white mt-auto relative overflow-hidden border-t border-white/5 spatial-ui-layer">
      ...
      <div className="container-xl pt-32 pb-20 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-12">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex flex-col gap-3">
              <motion.a
                whileHover={{ x: 5 }}
                href="https://wa.me/237654749357" target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-3 text-slate-400 hover:text-[var(--red)] transition-all group">
                <span className="w-9 h-9 rounded-xl bg-white/5 border border-white/5 group-hover:border-[var(--red)]/40 flex items-center justify-center shrink-0 transition-colors glass-morphism shadow-sm">
              ...
              <motion.a
                whileHover={{ x: 5 }}
                href="mailto:cust_care@ejs-cm.com"
                className="flex items-center gap-3 text-slate-400 hover:text-[var(--red)] transition-all group">
                <span className="w-9 h-9 rounded-xl bg-white/5 border border-white/5 group-hover:border-[var(--red)]/40 flex items-center justify-center shrink-0 transition-colors glass-morphism shadow-sm">
                  <Mail className="w-4 h-4" />
                </span>
                <span className="text-xs font-black uppercase tracking-widest">cust_care@ejs-cm.com</span>
              </motion.a>
            </div>
          </div>
        ...
        <div className="border-t border-white/5 mt-20 pt-10 flex flex-col md:flex-row justify-between items-center gap-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
          <p>&copy; {new Date().getFullYear()} E-JARNAULD SOFT. <span className="text-[var(--red)]/60">{t.footer.copyright}</span></p>
          <div className="flex gap-10">
            <Link href="/privacy" className="hover:text-white transition-all hover:tracking-widest">{language === "fr" ? "Confidentialité" : "Privacy"}</Link>
            <Link href="/terms" className="hover:text-white transition-all hover:tracking-widest">{language === "fr" ? "CGV / CGU" : "Terms"}</Link>
          </div>
        </div>
      </div>
    </footer>
